										...
										<table id="attendenceDetailedTable" class="display responsive nowrap" cellspacing="0" width="100%">
											<thead>
												<tr>
                                                    ...
                                                    <th><i class="fa fa-book"></i>LINK</th>
												</tr>
											</thead>
											<tbody>
											<?php
                                                ...
                                                $sqlSelectSubject = "SELECT subject_id FROM `enrollment` WHERE student_id='$student_id'";
                                                $resultSelectSubject = mysqli_query($connection, $sqlSelectSubject);
                                                while($rowSelectSubject = $resultSelectSubject->fetch_assoc()) {
                                                        $subject_id = $rowSelectSubject['subject_id'];
                                                        $sqlTute = "SELECT * FROM `tutes` WHERE subject_id = '$subject_id'";
                                                        $resultTute = mysqli_query($connection, $sqlTute);
                                                        while($row = $resultTute->fetch_assoc()) {
                                                         echo 
                                                            "<tr>
                                                            
                                                                <td> ".$row['tute_name']."</td>";
                                                                $sqlLecturers = "SELECT * FROM `tutes` WHERE subject_id= '$subject_id'";
                                                                ...
                                                                <td><a href=\"../uploads/Tutes/".$row['pdf']."\" target=\"_blank\" \">Download</a></td>
                                                            </tr>";
